# Training Perceiver model (https://huggingface.co/docs/transformers/model_doc/perceiver) used in using 
# RAVDESS data: https://zenodo.org/records/1188976
# Viper code base: https://github.com/VaianiLorenzo/ViPER/tree/main
...
config = PerceiverConfig(d_model=token_size, num_labels=2)   # 8 labels
...
# change the emotion labels here for RAVDESS DATA
...
#labels from RAVDESS DATA
train_file_list = [str(file)+".pt" for file in list(splits["Train"].File_ID)]
#train_labels = torch.tensor(splits["Train"][['neutral', 'calm', 'happy', 'sad', 'angry', 'fearful', 'disgust', 'surprised']].values)
train_labels = torch.tensor(splits["Train"][['lie', 'truth']].values)
...
val_labels = torch.tensor(splits["Val"][['lie', 'truth']].values)
...
# dirty trick make all visual feature 31 dimension

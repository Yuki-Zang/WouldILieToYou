# # finetuning Perceiver model (https://huggingface.co/docs/transformers/model_doc/perceiver) trained on RAVDESS DATASET
# RAVDESS data: https://zenodo.org/records/1188976
# output model from viper_train.py is added to huggingface model suites for emotion recognition applications
...
import pandas as pd
import numpy as np
from transformers import PerceiverConfig, PerceiverModel, PerceiverForSequenceClassification, PreTrainedModel
...
model = AutoModel.from_pretrained("./emotion-recognition", num_labels = 2, ignore_mismatched_sizes=True)
...
train_file_list = [str(file)+".pt" for file in list(splits["Train"].File_ID)]

#modified for deception detection
train_labels = torch.tensor(splits["Train"][['truth', 'lie']].values)


val_file_list = [str(file)+".pt" for file in list(splits["Val"].File_ID)]
#val_labels = torch.tensor(splits["Val"][['neutral', 'calm', 'happy', 'sad', 'angry', 'fearful', 'disgust', 'surprised']].values)
val_labels = torch.tensor(splits["Val"][['truth', 'lie']].values)
...
# dirty trick to make all visual feature 31 dimension
#loading data
train_data = None
val_data = None
if args.visual:
    train_data = torch.stack([torch.load(args.visual_features_input_folder + "/" + file)[:31, :] for file in train_file_list])
    val_data = torch.stack([torch.load(args.visual_features_input_folder + "/" + file)[:31, :] for file in val_file_list])
if args.audio:
    if train_data == None:
        train_data = torch.stack([torch.load(args.audio_features_input_folder + "/" + file)[:31, :] for file in train_file_list])
        val_data = torch.stack([torch.load(args.audio_features_input_folder + "/" + file)[:31, :] for file in val_file_list])
    else:
        train_data = torch.cat((train_data, torch.stack([torch.load(args.audio_features_input_folder + "/" + file)[:31, :] for file in train_file_list])), 2)
        val_data = torch.cat((val_data, torch.stack([torch.load(args.audio_features_input_folder + "/" + file)[:31, :] for file in val_file_list])), 2)

if args.textual:
    if train_data == None:
        train_data = torch.stack([torch.cat((torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                            ...
                                            torch.load(args.textual_features_input_folder + "/" + file),
                                             ...
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                  torch.load(args.textual_features_input_folder + "/" + file)), 0) for file in train_file_list])
        val_data = torch.stack([torch.cat((torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                            ...
                                            torch.load(args.textual_features_input_folder + "/" + file),
                                             ...
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                  torch.load(args.textual_features_input_folder + "/" + file)), 0) for file in val_file_list])
    else:
        train_data = torch.cat((train_data, torch.stack([torch.cat((torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                            ...
                                            torch.load(args.textual_features_input_folder + "/" + file),
                                             ...
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                  torch.load(args.textual_features_input_folder + "/" + file)), 0) for file in train_file_list])), 2)
        val_data = torch.cat((val_data, torch.stack([torch.cat((torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                            ...
                                            torch.load(args.textual_features_input_folder + "/" + file),
                                             ...
                                             torch.load(args.textual_features_input_folder + "/" + file),
                                              torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                torch.load(args.textual_features_input_folder + "/" + file),
                                                  torch.load(args.textual_features_input_folder + "/" + file)), 0) for file in val_file_list])), 2
